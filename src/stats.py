...
def main():
    ...
    if len(sys.argv) < 2:
        print("用法: python stats.py <command> [args...]")
        print("命令:")
        print("  project_stats    - 显示项目统计")
        print("  unclassified     - 显示未分类项目数量")
        print("  classification   - 显示分类统计")
        print("  doc_stats        - 显示文档统计")
        print("  commit_msg <mode> <run> [skip_ai] - 生成提交信息")
        return
    ...
    if command == "project_stats":
        ...
        if len(sys.argv) < 4:
            print("用法: python stats.py commit_msg <mode> <run> [skip_ai]")
            return
        
        mode = sys.argv[2]
        run = sys.argv[3]
        skip_ai = len(sys.argv) > 4 and sys.argv[4].lower() == "true"
        
        msg = reporter.generate_commit_message(mode, run, skip_ai)
        print(msg)
    else:
